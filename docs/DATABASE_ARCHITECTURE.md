...
- **`products`** — what is sold (`slug`, `kind ∈ {course,challenge,membership,single}`, `title`, `price_cents`, `currency`, `is_active`, `config jsonb`). RLS: public select. No Stripe ids are stored — Checkout defines the price inline (`price_data`) from `price_cents`/`currency`, attached to a Stripe Product whose id = the slug (auto-created per mode on first sale), so the same row sells in test and live mode alike (`20260710000000` dropped the former `stripe_price_id`/`stripe_product_id` columns).
...
- **`instructor_billing`** change (`20260728000000`): `iban`/`account_holder` now nullable — Connect-country instructors give their bank account to Stripe during hosted onboarding; the fields remain required (form + API) for manual-rail (out-of-region) instructors.
...
`workout_progress`, `checkout_recovery`, `email_opt_outs`, `leads` — commerce/lifecycle tables tied to `auth.users`, not instructors. **Not yet built:** `cohorts` (multi-week programs). (Stripe Connect tables landed in `20260728000000` — see `instructor_connect_accounts` above.)
